...
import HT.Types.{AsmBlock, Bool, Cacheline, ControlflowInst, HTArray, UInt, UInt64, types}
...
def test105body = {
  ...
  // GlobalPass(ast) // compiler not included in this demonstration
  ...
  val vcd_path = "/PATH/TO/test.vcd"
  //GlobalIte = 0
  println(s"Post XSRun Current time cost: ${System.currentTimeMillis() - startTime} ms")
  val test_block_pc = 0x1FFFFFCCL // observation.getPC(refo("test_start"))
  val test_block_ret_pc = 0x20000012L // observation.getPC(refo("test_end"))
  ...
  Range(blockInIbufStart, blockEnd, "block in exec") {
    entry {
      ...
      tillOnceWithBreak(blockCommit, redirectBeforeBlock, "prepare block exec") {
        entry {
      ...
      if (blockCommitted != 0x7fffffff) {
        tillOnceWithBreak(mainBlockCommit, redirectBeforeBlock, "test block exec") {
          entry {
